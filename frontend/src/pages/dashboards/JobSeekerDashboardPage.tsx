                ...
                return (
                  <div key={app._id} className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-gray-50/50 transition-colors">
                    <div className="flex items-start gap-3">
                      {app.jobId?.employerId?.logoUrl ? (
                        <img 
                          src={app.jobId.employerId.logoUrl.startsWith('http') ? app.jobId.employerId.logoUrl : `http://localhost:5000${app.jobId.employerId.logoUrl}`} 
                          alt={`${company} logo`} 
                          className="w-10 h-10 rounded-lg object-cover border border-gray-200 shrink-0 bg-white hidden sm:block"
                        />
                      ) : (
                        <div className="w-10 h-10 rounded-lg bg-gray-100 border border-gray-200 flex items-center justify-center shrink-0 hidden sm:flex">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-xs text-gray-500 flex flex-wrap items-center gap-x-3 gap-y-1">
                          <span className="flex items-center space-x-1">
                            ...
                            <Link 
                              to={`/employers/${app.jobId.employerId?._id || app.jobId.employerId}`}
                          ...
                          {app.jobId?.employerId?.userId?.name && (
                            <span className="flex items-center space-x-1">
                              <UserIcon className="w-3.5 h-3.5 text-gray-400" />
                              <span>Posted by: {app.jobId.employerId.userId.name}</span>
                            </span>
                          )}
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex flex-col space-y-0.5 mt-1">
                          <div className="text-xs font-semibold text-gray-600 flex items-center space-x-2">
                            <Link 
                              to={`/employers/${job.employerId?._id || job.employerId}`}
                              className="flex items-center space-x-1.5 hover:text-blue-600 transition-colors group w-fit"
                              onClick={(e) => e.stopPropagation()}
                            >
                              <Building className="w-3.5 h-3.5 text-gray-400 group-hover:text-blue-500" />
                          ...
                          {job.employerName && (
                            <p className="text-[10px] font-medium text-gray-500 flex items-center space-x-1.5">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
            ...
            <div className="p-6 space-y-5">
              <div className="flex justify-between items-start">
                <div className="flex items-start gap-3">
                  {selectedApp.jobId?.employerId?.logoUrl ? (
                    <img 
                      src={selectedApp.jobId.employerId.logoUrl.startsWith('http') ? selectedApp.jobId.employerId.logoUrl : `http://localhost:5000${selectedApp.jobId.employerId.logoUrl}`} 
                      alt="Company logo" 
