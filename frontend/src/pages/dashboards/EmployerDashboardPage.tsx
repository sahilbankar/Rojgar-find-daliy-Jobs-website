...
export const EmployerDashboard: React.FC = () => {
  ...
  const handleProfileSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let res = await employersAPI.updateProfile({
      ...
      if (profileForm.name !== user?.name || profileForm.email !== user?.email) {
        await usersAPI.updateProfile({ name: profileForm.name, email: profileForm.email });
      }
      ...
      // Reload page to refresh auth context or keep state
  ...
  const handleJobSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: Record<string, unknown> = {
        ...
        salaryRange,
        minSalary: postForm.minSalary ? Number(postForm.minSalary) : 0,
        maxSalary: postForm.maxSalary ? Number(postForm.maxSalary) : 0,
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input type="text" value={profileForm.name} onChange={e => setProfileForm({...profileForm, name: e.target.value})} className={FIELD_CLS} required />
              </div>
              <div>
                <label className="text-xs font-bold text-gray-700 uppercase">Contact Email</label>
                <input type="email" value={profileForm.email} onChange={e => setProfileForm({...profileForm, email: e.target.value})} className={FIELD_CLS} required />
              </div>
              <div>
